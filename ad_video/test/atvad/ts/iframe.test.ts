...
describe('mainExecのテスト', () => {
  ...
  test('正常_notviewthrough_pc', async () => {
    ...
    await iframe.mainExec(testScriptElement, rkValue, window, apiDomain, htmlDomain);
...
describe('mkIframeViaServerのテスト', () => {
  test('正常_notviewthrough_sp', async () => {
    ...
    const result: Jsontype = await (iframe as any).mkIframeViaServer(apiDomain, scriptElement, rkValue);
...
describe('mkIframeのテスト', () => {
  ...
  test('正常_notviewthrough_pc', async () => {
    ...
    const rkValue = '010011a1';
    const testScriptElement = document.getElementById('mkIframeTest');
    await (iframe as any).mkIframe(apiDomain, htmlDomain, testScriptElement, rkValue, (iframe as any).mkIframeViaServer);
